...
@allure.description("Подключение настроек СД PonyExpress по агрегации")
def test_aggregation_delivery_services(app):
    pony_express = app.service.post_delivery_services_pony_express(aggregation=True)
...
def test_moderation_delivery_services(admin):
    moderation = admin.connection.post_connections_pony_express()
...
def test_create_order_courier(app, payment_type, connections):
    new_order = app.order.post_single_order(payment_type=payment_type, type_ds="Courier", service="PonyExpress",
                                            declared_value=500, country_code="KZ", tariff="Express")
