...
def test_aggregation_delivery_services(app, admin, shop_id):
    CommonConnections.connect_aggregation_services_common(app=app, admin=admin, shop_id=shop_id,
                                                          connection_settings=app.settings.pony_express(
                                                              aggregation=True),
                                                          moderation_settings=admin.moderation.pony_express)
...
def test_create_order_courier(app, shop_id, warehouse_id_kz, payment_type, connections, shared_data):
    CommonOrders.test_single_order_common(app=app, connections=connections, shop_id=shop_id,
                                          ...
                                          tariff="Express", shared_data=shared_data["pony_express_a"]["order_ids"])


@allure.description("Получение списка заказов CД PonyExpress")
def test_get_orders(app, shared_data):
    CommonOrders.test_get_orders_common(app=app, shared_delivery_service="pony_express_a", shared_data=shared_data)


@allure.description("Получение информации о заказе CД PonyExpress")
def test_get_order_by_id(app, shared_data):
    CommonOrders.test_get_order_by_id_common(app=app, shared_data=shared_data["pony_express_a"]["order_ids"])
...
def test_order_status(app, shared_data):
    CommonOrders.test_order_status_common(app=app, shared_data=shared_data["pony_express_a"]["order_ids"])
...
def test_get_labels_out_of_parcel(app, shared_data):
    CommonOrders.test_get_labels_out_of_parcel_common(app=app,
                                                      shared_data=shared_data["pony_express_a"]["order_ids"])


@allure.description("Получение подробной информации о заказе СД PonyExpress")
def test_order_details(app, shared_data):
    CommonOrders.test_order_details_common(app=app, shared_data=shared_data["pony_express_a"]["order_ids"])


@allure.description("Создание партии СД PonyExpress")
def test_create_parcel(app, shared_data):
    CommonParcels.create_parcel_common(app=app, shared_delivery_service="pony_express_a", shared_data=shared_data)


@allure.description("Получение списка партий CД PonyExpress")
def test_get_parcels(app, shared_data):
    CommonParcels.test_get_parcels_common(app=app, shared_delivery_service="pony_express_a",
                                          shared_data=shared_data)


@allure.description("Получение информации о партии CД PonyExpress")
def test_get_parcel_by_id(app, shared_data):
    CommonParcels.test_get_parcel_by_id_common(app=app, shared_data=shared_data["pony_express_a"]["parcel_ids"])
...
def test_add_order_in_parcel(app, connections, shared_data):
    CommonParcels.add_order_in_parcel_common(app=app, connections=connections,
                                             shared_delivery_service="pony_express_a", shared_data=shared_data)


@allure.description("Получение этикетки СД PonyExpress")
def test_get_label(app, shared_data):
    CommonParcels.test_get_label_common(app=app, shared_data=shared_data["pony_express_a"]["order_ids_in_parcel"])


@allure.description("Получение АПП СД PonyExpress")
def test_get_app(app, shared_data):
    CommonParcels.test_get_app_common(app=app, shared_data=shared_data["pony_express_a"]["parcel_ids"])


@allure.description("Получение документов СД PonyExpress")
def test_get_documents(app, shared_data):
    CommonParcels.test_get_documents_common(app=app, shared_data=shared_data["pony_express_a"]["parcel_ids"])
...
def test_remove_order_in_parcel(app, connections, shared_data):
    CommonParcels.test_remove_order_in_parcel_common(app=app, connections=connections,
                                                     shared_delivery_service="pony_express_a",
                                                     shared_data=shared_data)
