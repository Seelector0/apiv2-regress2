...
@allure.description("Подключение настроек службы доставки СД TopDelivery по агрегации")
def test_integration_delivery_services(app, shop_id):
    topdelivery = app.service.post_delivery_service(shop_id=shop_id,
                                                    delivery_service=app.settings.topdelivery(aggregation=True))
...
def test_moderation_delivery_services(admin, shop_id):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.topdelivery(shop_id=shop_id))
...
def test_delivery_service_points(app, shop_id):
    delivery_service_points = app.info.get_delivery_service_points(shop_id=shop_id, delivery_service_code="TopDelivery")
...
def test_delivery_time_schedules(app, shop_id):
    delivery_time_schedules = app.info.get_delivery_time_schedules(shop_id=shop_id, delivery_service_code="TopDelivery")
...
def test_offers_courier(app, shop_id, warehouse_id, payment_type):
    offers_courier = app.offers.get_offers(shop_id=shop_id, warehouse_id=warehouse_id, payment_type=payment_type,
                                           types="Courier", delivery_service_code="TopDelivery")
...
def test_offers_delivery_point(app, shop_id, warehouse_id, payment_type):
    offers_delivery_point = app.offers.get_offers(shop_id=shop_id, warehouse_id=warehouse_id, payment_type=payment_type,
                                                  types="DeliveryPoint", delivery_service_code="TopDelivery",
...
def test_create_multi_order_courier(app, shop_id, warehouse_id, payment_type, connections, shared_data):
    new_order = app.order.post_multi_order(shop_id=shop_id, warehouse_id=warehouse_id, payment_type=payment_type,
                                           type_ds="Courier", service="TopDelivery", declared_value=500)
...
def test_create_multi_order_delivery_point(app, shop_id, warehouse_id, payment_type, connections, shared_data):
    new_order = app.order.post_multi_order(shop_id=shop_id, warehouse_id=warehouse_id, payment_type=payment_type,
                                           type_ds="DeliveryPoint", service="TopDelivery", delivery_point_code="55",
                                           declared_value=500)
    Checking.check_status_code(response=new_order, expected_status_code=201)
    Checking.checking_json_key(response=new_order, expected_value=INFO.created_entity)
    order_id = new_order.json()["id"]
...
def test_patch_single_order(app, shared_data):
    random_order_id = choice(shared_data["order_ids_single"])
...
def test_create_order_from_file(app, shop_id, warehouse_id, file_extension, connections, shared_data):
    new_order = app.order.post_import_order_format_metaship(shop_id=shop_id, warehouse_id=warehouse_id,
                                                            code="topdelivery", file_extension=file_extension)
