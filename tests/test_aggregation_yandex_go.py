...
@allure.description("Подключение настроек службы доставки СД YandexGo по агрегации")
def test_integration_delivery_services(app, shop_id):
    yandex_go = app.service.post_delivery_service(shop_id=shop_id,
                                                  delivery_service=app.settings.yandex_go(aggregation=True))
...
def test_moderation_delivery_services(admin, shop_id):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.yandex_go(shop_id=shop_id))
