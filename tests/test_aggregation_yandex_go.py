...
def test_integration_delivery_services(app):
    yandex_go = app.service.post_delivery_service(delivery_service=app.settings.yandex_go(aggregation=True))
...
def test_moderation_delivery_services(admin):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.yandex_go())
