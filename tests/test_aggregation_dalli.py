...
@allure.description("Подключение настроек службы доставки СД Dalli по агрегации")
def test_integration_delivery_services(app, shop_id):
    dalli = app.service.post_delivery_service(shop_id=shop_id, delivery_service=app.settings.dalli(aggregation=True))
...
def test_moderation_delivery_services(admin, shop_id):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.dalli(shop_id=shop_id))
