...
def test_create_multi_order(app, shop_id, warehouse_id, payment_type, connections, shared_data):
    if payment_type == "Paid":
        CommonOrders.test_multi_order_common(app=app, connections=connections, shop_id=shop_id,
                                             ...
                                             price_1=0, price_2=0, shared_data=shared_data["l_post_i"]["order_ids"])
    else:
        CommonOrders.test_multi_order_common(app=app, connections=connections, shop_id=shop_id,
                                             warehouse_id=warehouse_id,
                                             payment_type=payment_type, delivery_type="Courier", service="LPost",
                                             price_1=500, price_2=500,
                                             shared_data=shared_data["l_post_i"]["order_ids"])
...
def test_create_order_courier(app, shop_id, warehouse_id, payment_type, connections, shared_data):
    if payment_type == "Paid":
        ...
        CommonOrders.test_single_order_common(app=app, connections=connections, shop_id=shop_id,
                                              warehouse_id=warehouse_id,
                                              payment_type=payment_type, delivery_type="Courier",
                                              service="LPost", shared_data=shared_data["l_post_i"]["order_ids"])


@allure.description("Получение списка заказов CД LPost")
def test_get_orders(app, shared_data):
    CommonOrders.test_get_orders_common(app=app, shared_delivery_service="l_post_i", shared_data=shared_data)


@allure.description("Получение информации о заказе CД LPost")
def test_get_order_by_id(app, shared_data):
    CommonOrders.test_get_order_by_id_common(app=app, shared_data=shared_data["l_post_i"]["order_ids"])


@allure.description("Редактирование заказа СД LPost")
def test_editing_order(app, shared_data):
    CommonOrders.test_editing_order_common(app=app, shared_data=shared_data["l_post_i"]["order_ids"])
...
def test_order_status(app, shared_data):
    CommonOrders.test_order_status_common(app=app, shared_data=shared_data["l_post_i"]["order_ids"])


@allure.description("Удаление заказа LPost")
def test_delete_order(app, connections, shared_data):
    CommonOrders.test_delete_order_common(app=app, connections=connections, shared_delivery_service="l_post_i",
                                          shared_data=shared_data)


@allure.description("Получение подробной информации о заказе СД LPost")
def test_order_details(app, shared_data):
    CommonOrders.test_order_details_common(app=app, shared_data=shared_data["l_post_i"]["order_ids"])


@allure.description("Создание партии CД LPost")
def test_create_parcel(app, shared_data):
    CommonParcels.create_parcel_common(app=app, shared_delivery_service="l_post_i", shared_data=shared_data)


@allure.description("Получение списка партий CД LPost")
def test_get_parcels(app, shared_data):
    CommonParcels.test_get_parcels_common(app=app, shared_delivery_service="l_post_i", shared_data=shared_data)


@allure.description("Получение информации о партии CД LPost")
def test_get_parcel_by_id(app, shared_data):
    CommonParcels.test_get_parcel_by_id_common(app=app, shared_data=shared_data["l_post_i"]["parcel_ids"])


@allure.description("Получение АПП СД LPost")
def test_get_app(app, shared_data):
    CommonParcels.test_get_app_common(app=app, shared_data=shared_data["l_post_i"]["parcel_ids"])
...
def test_forms_parcels_labels(app, shared_data):
    CommonParcels.test_forms_parcels_labels_common(app=app, shared_data=shared_data["l_post_i"]["parcel_ids"])


@allure.description("Получение документов СД LPost")
def test_get_documents(app, shared_data):
    CommonParcels.test_get_documents_common(app=app, shared_data=shared_data["l_post_i"]["parcel_ids"])
