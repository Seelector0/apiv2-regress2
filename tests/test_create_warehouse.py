...
class TestWarehouses:
    ...
    def test_create_new_warehouse(self, token, connections):
        old_warehouse_list = connections.get_warehouses_list()
        result_post = ApiWarehouse.create_warehouse(fullname="Виктор Викторович", headers=token)
        new_warehouse_list = connections.get_warehouses_list()
        ...
        Checking.checking_sum_len_lists(old_list=old_warehouse_list, new_list=new_warehouse_list)
    ...
    def test_put_warehouse(self, connections, token):
        ...
        for element in warehouse:
            ...
            result_put = ApiWarehouse.put_warehouse_by_id(headers=token, warehouse_id=uuid, fullname="Ваня Ваня")
