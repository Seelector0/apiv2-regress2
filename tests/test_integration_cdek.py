...
def test_integration_delivery_services(app, shop_id):
    cdek = app.service.post_delivery_service(shop_id=shop_id, delivery_service=app.settings.cdek())
...
def test_get_original_labels_out_of_parcel(app, connections, format_, shared_data):
    combined_order_ids = shared_data["order_ids"] + shared_data["order_ids_single"]
    for order_id in combined_order_ids:
