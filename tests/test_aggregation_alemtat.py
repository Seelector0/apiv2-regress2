...
def test_aggregation_delivery_services(app, shop_id):
    alemtat = app.service.post_delivery_service(shop_id=shop_id,
                                                delivery_service=app.settings.alemtat(aggregation=True))
...
def test_update_connection_id(admin, shop_id):
    update = admin.connection.put_update_connection_id(shop_id=shop_id, delivery_service="AlemTat",
                                                       settings=admin.dicts.form_settings_ds_alemtat())
...
def test_moderation_delivery_services(admin, shop_id):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.alemtat(shop_id=shop_id))
...
@allure.description("Получение Courier оферов по СД AlemTat")
...
def test_offers_courier(app, shop_id, warehouse_id_kz, payment_type):
    offers_delivery_point = app.offers.get_offers(shop_id=shop_id, warehouse_id=warehouse_id_kz,
                                                  ...
                                                  delivery_service_code="AlemTat", country_code="KZ")
    Checking.check_status_code(response=offers_delivery_point, expected_status_code=200)
    Checking.checking_json_key(response=offers_delivery_point, expected_value=["Courier"])
...
def test_create_order_courier(app, shop_id, warehouse_id_kz, payment_type, connections, shared_data):
    new_order = app.order.post_single_order(shop_id=shop_id, warehouse_id=warehouse_id_kz, payment_type=payment_type,
                                            type_ds="Courier", service="AlemTat",
