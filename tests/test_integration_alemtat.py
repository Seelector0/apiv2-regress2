...
@allure.description("Подключение настроек СД AlemTat")
def test_integration_delivery_services(app):
    alemtat = app.service.post_delivery_services_alemtat()
...
def test_offers_courier(app, payment_type):
    offers_delivery_point = app.offers.get_offers(payment_type=payment_type, types="Courier",
...
def test_create_order_courier(app, payment_type, connections):
    new_order = app.order.post_single_order(payment_type=payment_type, type_ds="Courier", service="AlemTat",
