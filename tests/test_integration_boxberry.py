...
def test_intake_offices(app, token):
    result_intake_offices = app.info.intake_offices(delivery_service_code="Boxberry", limit=10, headers=token)
...
def test_info_vats(app, token):
    result_info_vats = app.info.info_vats(delivery_service_code="Boxberry", headers=token)
...
def test_info_statuses(app, token):
    result_info_delivery_service_services = app.info.info_delivery_service_services(code="Boxberry", headers=token)
...
def test_create_parcel(app, token):
    ...
    result_create_parcel = app.parcel.create_parcel(all_orders=True, order_id=orders_id, headers=token)
