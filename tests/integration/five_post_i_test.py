...
def test_get_orders(app, shared_data):
    CommonOrders.test_get_orders_common(app=app, shared_delivery_service="five_post_i", shared_data=shared_data)


@allure.description("Получение информации о заказе CД FivePost")
def test_get_order_by_id(app, shared_data):
    CommonOrders.test_get_order_by_id_common(app=app, shared_data=shared_data["five_post_i"]["order_ids"])
...
def test_order_status(app, shared_data):
    CommonOrders.test_order_status_common(app=app, shared_data=shared_data["five_post_i"]["order_ids"])
...
def test_generate_security_code(app, shared_data):
    CommonOrders.test_generate_security_common(app=app, shared_data=shared_data["five_post_i"]["order_ids"])


@allure.description("Получения этикетки СД FivePost вне партии")
def test_get_labels_out_of_parcel(app, shared_data):
    CommonOrders.test_get_labels_out_of_parcel_common(app=app, shared_data=shared_data["five_post_i"]["order_ids"])


@allure.description("Получение подробной информации о заказе СД FivePost")
def test_order_details(app, shared_data):
    CommonOrders.test_order_details_common(app=app, shared_data=shared_data["five_post_i"]["order_ids"])


@allure.description("Отмена заказа СД FivePost")
def test_patch_order_cancelled(app, connections, shared_data):
    CommonOrders.test_patch_order_cancelled_common(app=app, delivery_service="five_post_i",
...
def test_delete_order(app, connections, shared_data):
    CommonOrders.test_delete_order_common(app=app, connections=connections, shared_delivery_service="five_post_i",
...
def test_get_parcels(app, shared_data):
    CommonParcels.test_get_parcels_common(app=app, shared_delivery_service="five_post_i", shared_data=shared_data)


@allure.description("Получение информации о партии CД FivePost")
def test_get_parcel_by_id(app, shared_data):
    CommonParcels.test_get_parcel_by_id_common(app=app, shared_data=shared_data["five_post_i"]["parcel_ids"])
...
def test_add_order_in_parcel(app, connections, shared_data):
    CommonParcels.add_order_in_parcel_common(app=app, connections=connections, shared_delivery_service="five_post_i",
...
def test_patch_weight_random_order_in_parcel(app, connections, shared_data):
    CommonParcels.test_patch_weight_random_order_in_parcel_common(app=app, connections=connections,
                                                                  shared_data=shared_data
                                                                  ["five_post_i"]["order_ids_in_parcel"])
...
def test_get_labels_from_parcel(app, shared_data):
    CommonParcels.test_get_labels_from_parcel_common(app=app, shared_delivery_service="five_post_i",
                                                     shared_data=shared_data)


@allure.description("Получение АПП СД FivePost")
def test_get_app(app, shared_data):
    CommonParcels.test_get_app_common(app=app, shared_data=shared_data["five_post_i"]["parcel_ids"])


@allure.description("Получение документов СД FivePost")
def test_get_documents(app, shared_data):
    CommonParcels.test_get_documents_common(app=app, shared_data=shared_data["five_post_i"]["parcel_ids"])
...
def test_forms_parcels_labels(app, shared_data):
    CommonParcels.test_forms_parcels_labels_common(app=app, shared_data=shared_data["five_post_i"]["parcel_ids"])
...
def test_remove_order_in_parcel(app, connections, shared_data):
    CommonParcels.test_remove_order_in_parcel_common(app=app, connections=connections,
                                                     shared_delivery_service="five_post_i", shared_data=shared_data)
