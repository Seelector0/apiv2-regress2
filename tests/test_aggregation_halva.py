...
@allure.description("Подключение настроек службы доставки СД Halva по агрегации")
def test_integration_delivery_services(app, shop_id):
    halva = app.service.post_delivery_service(shop_id=shop_id, delivery_service=app.settings.halva(aggregation=True))
...
def test_moderation_delivery_services(admin, shop_id):
    moderation = admin.connection.post_connections(delivery_service=admin.moderation.halva(shop_id=shop_id))
...
def test_delivery_service_points(app, shop_id):
    delivery_service_points = app.info.get_delivery_service_points(shop_id=shop_id, delivery_service_code="Halva")
...
def test_create_order_delivery_point(app, shop_id, warehouse_id, payment_type, connections, shared_data):
    new_order = app.order.post_single_order(shop_id=shop_id, warehouse_id=warehouse_id, payment_type=payment_type,
                                            type_ds="DeliveryPoint", service="Halva", delivery_point_code="Халва-6193",
                                            declared_value=500)
