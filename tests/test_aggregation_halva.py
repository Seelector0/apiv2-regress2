...
def test_delivery_service_points(app, shop_id):
    app.tests_info.test_delivery_service_points_common(shop_id=shop_id, delivery_service_code="Halva")
...
def test_editing_order(app, shared_data):
    CommonOrders.test_editing_order_common(app=app, shared_data=shared_data["halva_a"]["order_ids"])


@allure.description("Получение списка заказов CД Halva")
def test_get_orders(app, shared_data):
    CommonOrders.test_get_orders_common(app=app, shared_delivery_service="halva_a", shared_data=shared_data)


@allure.description("Получение информации о заказе CД Halva")
def test_get_order_by_id(app, shared_data):
    CommonOrders.test_get_order_by_id_common(app=app, shared_data=shared_data["halva_a"]["order_ids"])
...
def test_order_status(app, shared_data):
    CommonOrders.test_order_status_common(app=app, shared_data=shared_data["halva_a"]["order_ids"])
...
def test_get_parcels(app, shared_data):
    CommonParcels.test_get_parcels_common(app=app, shared_delivery_service="halva_a", shared_data=shared_data)


@allure.description("Получение информации о партии CД Halva")
def test_get_parcel_by_id(app, shared_data):
    CommonParcels.test_get_parcel_by_id_common(app=app, shared_data=shared_data["halva_a"]["parcel_ids"])
...
def test_get_labels_from_parcel(app, shared_data):
    CommonParcels.test_get_labels_from_parcel_common(app=app, shared_delivery_service="halva_a",
                                                     shared_data=shared_data)


@allure.description("Получение АПП СД Halva")
def test_get_app(app, shared_data):
    CommonParcels.test_get_app_common(app=app, shared_data=shared_data["halva_a"]["parcel_ids"])


@allure.description("Получение документов СД Halva")
def test_get_documents(app, shared_data):
    CommonParcels.test_get_documents_common(app=app, shared_data=shared_data["halva_a"]["parcel_ids"])


@allure.description("Создание формы с этикетками партии СД Halva")
@pytest.mark.not_parallel
def test_forms_parcels_labels(app, shared_data):
    CommonParcels.test_forms_parcels_labels_common(app=app, shared_data=shared_data["halva_a"]["parcel_ids"])
...
def test_remove_order_in_parcel(app, connections, shared_data):
    CommonParcels.test_remove_order_in_parcel_common(app=app, connections=connections,
                                                     shared_delivery_service="halva_a", shared_data=shared_data)
