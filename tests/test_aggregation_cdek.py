...
def test_aggregation_delivery_services(app, admin, shop_id):
    CommonConnections.connect_aggregation_services_common(app=app, admin=admin, shop_id=shop_id,
                                                          connection_settings=app.settings.cdek(aggregation=True),
                                                          moderation_settings=admin.moderation.cdek)
...
def test_get_order_by_id(app, shared_data):
    CommonOrders.test_get_order_by_id_common(app=app, shared_data=shared_data["cdek_a"]["order_ids"])
...
def test_order_status(app, shared_data):
    CommonOrders.test_order_status_common(app=app, shared_data=shared_data["cdek_a"]["order_ids"])
...
def test_delete_order(app, connections, shared_data):
    CommonOrders.test_delete_order_common(app=app, connections=connections,
                                          shared_delivery_service="cdek_a", shared_data=shared_data)
...
def test_get_labels_out_of_parcel(app, labels, shared_data):
    combined_order_ids = shared_data["order_ids"] + shared_data["cdek_a"]["order_ids_single"]
...
def test_get_labels_out_of_parcel(app, shared_data):
    combined_order_ids = shared_data["cdek_a"]["order_ids"] + shared_data["cdek_a"]["order_ids_single"]
    CommonOrders.test_get_labels_out_of_parcel_common(app=app, shared_data=combined_order_ids)
...
def test_create_parcel(app, shared_data):
    CommonParcels.create_parcel_common(app=app, shared_data=shared_data, shared_delivery_service="cdek_a",
                                       types="order_ids_single")


@allure.description("Получение списка партий CД Cdek")
def test_get_parcels(app, shared_data):
    CommonParcels.test_get_parcels_common(app=app, shared_delivery_service="cdek_a", shared_data=shared_data)


@allure.description("Получение информации о партии CД Cdek")
def test_get_parcel_by_id(app, shared_data):
    CommonParcels.test_get_parcel_by_id_common(app=app, shared_data=shared_data["cdek_a"]["parcel_ids"])
...
def test_add_order_in_parcel(app, connections, shared_data):
    CommonParcels.add_order_in_parcel_common(app=app, connections=connections, shared_delivery_service="cdek_a",
                                             shared_data=shared_data)
...
def test_get_label(app, labels, shared_data):
    CommonParcels.test_get_label_common(app=app, labels=labels,
                                        shared_data=shared_data["cdek_a"]["order_ids_in_parcel"])


@allure.description("Получение этикеток СД Cdek")
def test_get_label(app, shared_data):
    CommonParcels.test_get_label_common(app=app, shared_data=shared_data["cdek_a"]["order_ids_in_parcel"])


@allure.description("Получение этикеток заказов из партии СД Cdek")
def test_get_labels_from_parcel(app, shared_data):
    CommonParcels.test_get_labels_from_parcel_common(app=app, shared_delivery_service="cdek_a", shared_data=shared_data)


@allure.description("Получение АПП СД Cdek")
def test_get_app(app, shared_data):
    CommonParcels.test_get_app_common(app=app, shared_data=shared_data["cdek_a"]["parcel_ids"])


@allure.description("Получение документов СД Cdek")
def test_get_documents(app, shared_data):
    CommonParcels.test_get_documents_common(app=app, shared_data=shared_data["cdek_a"]["parcel_ids"])


@allure.description("Создание формы с этикетками партии СД Cdek")
@pytest.mark.not_parallel
def test_forms_parcels_labels(app, shared_data):
    CommonParcels.test_forms_parcels_labels_common(app=app, shared_data=shared_data["cdek_a"]["parcel_ids"])
...
def test_remove_order_in_parcel(app, connections, shared_data):
    CommonParcels.test_remove_order_in_parcel_common(app=app, connections=connections, shared_delivery_service="cdek_a",
                                                     shared_data=shared_data)
